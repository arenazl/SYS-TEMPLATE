"""API Usuario - Generado automáticamente"""
...
def serialize(item: Usuario) -> dict:
...
async def listar(
    ...
    query = select(Usuario).options(selectinload(Usuario.organizacion)).where(
    ...
    query = query.offset(skip).limit(limit).order_by(Usuario.id.desc())
...
async def obtener(
    ...
    result = await db.execute(
        select(Usuario).options(selectinload(Usuario.organizacion)).where(
            Usuario.id == id,
            Usuario.organizacion_id == current_user.organizacion_id
        )
    )
...
async def crear(
    ...
    item = Usuario(**data.model_dump(), organizacion_id=current_user.organizacion_id)
    db.add(item)
    await db.commit()
