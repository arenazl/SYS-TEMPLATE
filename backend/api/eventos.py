"""API Evento - Generado automáticamente"""
...
async def listar(
    ...
    query = select(Evento).where(
        Evento.organizacion_id == current_user.organizacion_id
    )
    if activo is not None:
        query = query.where(Evento.activo == activo)

    count_query = select(func.count()).select_from(query.subquery())
    total = await db.scalar(count_query)

    query = query.offset(skip).limit(limit).order_by(Evento.id.desc())
...
async def crear(
    ...
    item = Evento(**data.model_dump(), organizacion_id=current_user.organizacion_id)
    db.add(item)
    await db.commit()
