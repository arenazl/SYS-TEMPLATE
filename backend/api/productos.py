"""API Producto - Generado automáticamente"""
...
async def listar(
    ...
    query = query.offset(skip).limit(limit).order_by(Producto.id.desc())
...
async def crear(
    ...
    item = Producto(**data.model_dump(), organizacion_id=current_user.organizacion_id)
    db.add(item)
    await db.commit()
