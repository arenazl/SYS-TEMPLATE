"""API Departamento - Generado automáticamente"""
...
from models.departamento import Departamento, DepartamentoCreate, DepartamentoUpdate
...
async def listar(
    ...
    query = select(Departamento).where(
        Departamento.organizacion_id == current_user.organizacion_id
    )
    ...
    query = query.offset(skip).limit(limit).order_by(Departamento.id.desc())
...
async def crear(
    ...
    item = Departamento(**data.model_dump(), organizacion_id=current_user.organizacion_id)
    db.add(item)
    await db.commit()
