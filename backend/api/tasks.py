"""API Task - Generado automáticamente"""
...
async def listar(
    ...
    query = select(Task).where(
        Task.organizacion_id == current_user.organizacion_id
    )
    if activo is not None:
        query = query.where(Task.activo == activo)
...
async def crear(
    ...
    item = Task(**data.model_dump(), organizacion_id=current_user.organizacion_id)
    db.add(item)
    await db.commit()
