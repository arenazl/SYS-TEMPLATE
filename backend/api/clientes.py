"""API Cliente - Generado automáticamente"""
...
async def listar(
    ...
    query = select(Cliente).where(
        Cliente.organizacion_id == current_user.organizacion_id
    )
    ...
    query = query.offset(skip).limit(limit).order_by(Cliente.id.desc())
...
async def crear(
    ...
    item = Cliente(**data.model_dump(), organizacion_id=current_user.organizacion_id)
    db.add(item)
    await db.commit()
