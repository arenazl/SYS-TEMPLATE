"""API Proveedor - Generado automáticamente"""
...
async def listar(
    ...
    query = select(Proveedor).where(
        Proveedor.organizacion_id == current_user.organizacion_id
    )
    ...
    query = query.offset(skip).limit(limit).order_by(Proveedor.id.desc())
...
async def crear(
    ...
    item = Proveedor(**data.model_dump(), organizacion_id=current_user.organizacion_id)
    db.add(item)
    await db.commit()
